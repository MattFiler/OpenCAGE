...
function BarChart({ chartId, data, label, title, yAxisLabel, xAxisLabel = 'Version', averageValue = null }) {
    // ... (BarChart component code remains the same) ...
            ...
            chartInstanceRef.current = new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        annotation: { annotations: annotations } // Keep annotation for average line
                    },
                    scales: {
...
function ScatterChart({ chartId, datasets, title, xAxisLabel, yAxisLabel, xAxisType = 'linear', yAxesConfig = {}, reverseXAxis = false, customAnnotations = {} }) {
    // ... (ScatterChart component code remains the same as previous version) ...
    const chartRef = React.useRef(null);
    const chartInstanceRef = React.useRef(null);
                        ...
                        tooltip: { 
                             callbacks: { 
                                title: function(tooltipItems) { 
                                    ...
                                    if (xAxisType === 'time' && point.originalReleaseDate && chartId === 'releaseTimelineChart') return point.originalReleaseDate; 
...
function HeatmapTimeline({ data }) { 
    // ... (HeatmapTimeline component code remains the same) ...
...
function TopUsersList({ data }) {
    // ... (TopUsersList component code remains the same) ...
    if (!data) return React.createElement(LoadingSpinner);
...
function StatItem({ label, value, isLoading, valueFontSize = '1.5rem' }) {
    // ... (StatItem component code remains the same) ...
...
function ChartCard({ chartId, title, children, isExpanded, onToggleExpand, isLoading, error, headerControls = null }) { 
    // ... (ChartCard component code remains the same) ...
...
function App() {
    ...
    const popularityChartDatasets = React.useMemo(() => { 
        ...
        datasets.push({ 
            ...
            showLine: true, tension: 0.1, pointRadius: 5, yAxisID: 'yPrimary' 
        });
    ...
    const popularityChartAnnotations = React.useMemo(() => { 
        ...
        return {
            alienRomulusMarker: {
                type: 'line', scaleID: 'x', value: romulusReleaseDate, 
                borderColor: 'rgba(220, 53, 69, 0.5)', borderWidth: 2, borderDash: [4, 4], 
                label: {
                    content: 'Alien Romulus Release', enabled: true, position: 'top', 
                    backgroundColor: 'rgba(220, 53, 69, 0.5)', color: 'white',
                    font: { style: 'bold', size: 10 }, yAdjust: -5, rotation: 0 
                }
            }
        };
    }, []); 
    
    // Constants for Chart IDs
    const CHART_IDS = {
    ...
    return React.createElement('div', { className: 'container pb-5' }, 
        ...
        // Row 3: Bar Charts (Using ChartCard)
